...
class ARDNS_FN_Quantum:
    def __init__(self, state_dim=2, action_dim=4, ms_dim=8, ml_dim=16,
                 ...
                 use_fn_stages=True, use_attention=True):
    ...
    def get_stage_parameters(self):
        current_stage = None
        for stage, (start, end) in self.fn_dv_stages.items():
            if start <= self.episode <= end:
                current_stage = stage
                break

        if current_stage == 'fn_sn':
            return {
...
def evaluate_statistical_significance(
    ...
    ardns_name="ARDNS-FN-Quantum", dqn_name="DQN", ppo_name="PPO", episode_range=None
):
    ...
    comparisons = [
        ('ARDNS-FN-Quantum vs DQN', perform_test(ardns_rewards, dqn_rewards, ardns_name, dqn_name, ardns_normal, dqn_normal)),
        ('ARDNS-FN-Quantum vs PPO', perform_test(ardns_rewards, ppo_rewards, ardns_name, ppo_name, ardns_normal, ppo_normal)),
...
if __name__ == "__main__":
    ...
    ardns_model = ARDNS_FN_Quantum(state_dim=2, action_dim=4, ms_dim=8, ml_dim=16,
                                 use_attention=True)
    dqn_model = DQN(state_dim=2, action_dim=4, hidden_dim=32)
    ppo_model = PPO(state_dim=2, action_dim=4, hidden_dim=32)

    print("Starting ARDNS-FN-Quantum simulation...")
    ardns_rewards, ardns_steps, ardns_goals = run_simulation(ardns_model, env, "ARDNS-FN-Quantum", episodes=20000)
    ...
    print("\nARDNS-FN-Quantum Final Results:")
    ...
    evaluate_statistical_significance(
        ardns_rewards, dqn_rewards, ppo_rewards, ardns_goals, dqn_goals, ppo_goals, success_reward=10.0,
        ardns_name="ARDNS-FN-Quantum", dqn_name="DQN", ppo_name="PPO", episode_range=(0, 20000)
    )
    ...
    plt.plot(episodes, ardns_smooth, label='ARDNS-FN-Quantum', color='blue', alpha=0.7)
    ...
    plt.plot(episodes, ardns_steps_smooth, label='ARDNS-FN-Quantum', color='blue', alpha=0.7)
    ...
    plt.plot(episodes, ardns_var_smooth, label='ARDNS-FN-Quantum', color='blue', alpha=0.7)
